...
# Map "d3" and "w2" to the closest DDGS timelimit + snippet keyword filter
_DDGS_TIMELIMIT_MAP = {
    "d":  "d",
    "d3": "d",   # fetch daily, user sees "3 days" — DDGS has no exact 3d limit
    "w":  "w",
    "w2": "m",   # fetch monthly, then we'll note it
    "m":  "m",
    None: None,
}
...
def search_one_portal(
    ...
    try:
        ...
        # Days → DDGS timelimit
        timelimit = _DDGS_TIMELIMIT_MAP.get(days_key, "m")

        # Retry up to 3 times with backoff to handle rate-limiting
        raw = []
        for attempt in range(3):
            try:
                time.sleep(attempt * 1.5)   # 0s, 1.5s, 3s
                with DDGS() as ddgs:
                    raw = list(ddgs.text(query, max_results=12, timelimit=timelimit) or [])
                if raw:
                    break
            except Exception:
                if attempt == 2:
        ...
        # Fallback: if strict path query returned nothing, try root domain query
        if not raw:
            fallback_q = f'site:{portal.lower()}.com "{role}" {location} jobs'
            try:
                time.sleep(1)
                with DDGS() as ddgs:
                    raw = list(ddgs.text(fallback_q, max_results=12, timelimit=timelimit) or [])
            except Exception:
                pass
